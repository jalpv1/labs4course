...
public class HospitalModel {
    ...
    public void Simulate(double timeModeling)
    {
        ...
        while (tCurrent < timeModeling)
        {
            ...
            elementsList.stream().filter(e->e.gettNext()<tNext).forEach(e-> {tNext = e.gettNext();
                event = elementsList.indexOf(e);});

            ManageChannels();
            System.out.println("It's time for event in "+ elementsList.get(event).getName()+" , time = " + elementsList.get(event).gettNext());
    ...
    public void printResult()
    {
        ...
        for (Element e:  elementsList)
        {
            ...
            if (e instanceof HospitalProcess)
            {
                ...
                //}
                process.types.forEach( t -> {types.set(t.index - 1, types.get(t.index - 1) +t.waitingTime);
            ...
            if (e instanceof Doctor)
            {
                ...
                doctor.getdTypes().forEach(t -> { types.set(t.index - 1, types.get(t.index - 1) + t.waitingTime);
                    quantities.set(t.index - 1, quantities.get(t.index - 1) + t.quantity);});
